    ...
    private void clickEliminar(MouseEvent event) {
        if(validarSeleccion()){
            Alert alerta = ControladorVentanas.crearAlerta("Confirmación",
                    "¿Está seguro que desea eliminar esta carrera?", Alert.AlertType.CONFIRMATION);
            Optional<ButtonType> respuesta = alerta.showAndWait();

            if(respuesta.get() == ButtonType.OK){
                int idCarrera = tabla.getSelectionModel().getSelectedItem().getIdCarrera();
                ...
                alerta = ControladorVentanas.crearAlerta("Operación exitosa",
                            "Se ha eliminado la carrera correctamente", Alert.AlertType.INFORMATION);
                alerta.showAndWait();
            }
        }
    }
